...
func (a *App) getCombinedFlagInfo(flagName string) (*CombinedFlagInfo, error) {
	...
	val, err := a.RedisClient.Get(context.Background(), cacheKey).Result()
	if err == nil {
		// Cache HIT
	...
	if err == nil {
		if err := a.RedisClient.Set(context.Background(), cacheKey, jsonData, CACHE_TTL).Err(); err != nil {
...
func (a *App) fetchFlag(flagName string) (*Flag, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Printf("erro ao fechar body do flag-service: %v", err)
		}
	}()
...
func (a *App) fetchRule(flagName string) (*TargetingRule, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Printf("erro ao fechar body do targeting-service: %v", err)
		}
	}()
